# AegisAuth Wiki

Welcome to the AegisAuth Wiki repository. This repository contains the complete documentation, configuration guides, and references for the AegisAuth Minecraft authentication plugin.

## Table of Contents

- [Home](Home.md) - Welcome page and project overview.
- [Features](Features.md) - Deep dive into Argon2id encryption, rate-limiting, and premium auto-login.
- [Installation Guide](Installation.md) - Setting up the plugin with SQLite or MySQL databases.
- [Configuration](Configuration.md) - Parameter explanations for config.yml.
- [Commands and Permissions](Commands-and-Permissions.md) - Command lists and permission nodes.
- [Developers Guide](Developers.md) - Build instructions and code architecture.

---

## Main Repository

For the source code and compilation assets of the AegisAuth plugin, please visit the primary repository:
https://github.com/EliteClockman-ctrl/AegisAuth

